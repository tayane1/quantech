...
class Conversation(models.Model):
    ...
    def __str__(self):
        ...
        try:
            ...
            if participants:
                participants_names = ", ".join([u.get_full_name() or u.username for u in participants])
    ...
    def clean(self):
        ...
        # Ne valider les participants que si l'objet est déjà sauvegardé
        # (les relations ManyToMany nécessitent un ID)
        if self.pk:
    ...
    def save(self, *args, **kwargs):
        ...
        # Sauvegarder d'abord pour obtenir un ID (nécessaire pour ManyToMany)
        is_new = self.pk is None
        super().save(*args, **kwargs)
        # Ensuite valider les participants (seulement si l'objet est sauvegardé)
        if not is_new:
            self.full_clean()
