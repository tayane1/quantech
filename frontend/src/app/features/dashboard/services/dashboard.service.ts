...
import { Announcement, Activity, Schedule, Notification } from '../../../core/models/dashboard.model';
...
export class DashboardService {
  ...
  getAnnouncements(params?: { published?: boolean; limit?: number }): Observable<{ results: Announcement[]; count: number }> {
    return this.api.get<{ results: Announcement[]; count: number } | Announcement[]>('announcement/announcements/visible-to-me/', params).pipe(
      // Transforme la réponse pour gérer les deux cas : paginée ou non paginée
      map(response => {
        // Si la réponse est directement un tableau (non paginée)
        ...
        // Si la réponse est paginée, elle est déjà dans le bon format
        return response as { results: Announcement[]; count: number };
      })
    );
  }
